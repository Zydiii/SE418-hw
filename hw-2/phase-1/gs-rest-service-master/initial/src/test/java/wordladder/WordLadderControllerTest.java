...
public class WordLadderControllerTest {
    ...
    public void testReturnedAns() throws Exception {
        ...
        MvcResult result = this.mockMvc.perform(get("/WordLadder").param("word1", start).param("word2", end)).andDo(print()).andExpect(status().isOk())
                .andReturn();

        WordLadder ladder_test = new WordLadder(start, end, dict);
